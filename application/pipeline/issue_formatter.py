class IssueFormatter:
    def extract_issue_data(issues):
        ...
        for issue in issues:
            ...
            issues_by_row[issue["row-number"]][issue["field"]][
                issue["issue-type"]
            ] = issue["value"]
    ...
    def map_issue(field, issue):
        ...
        if "missing" in keys:
            ...
            return (
                issue[keys[0]],
                "invalid",
                f"Value provided is below the minimum allowed",
            )
            ...
            for field in issues[row].keys():
                ...
                if field == "GeoX,GeoY":
                    ...
                    if "outside England" in keys[0]:
                        geox_issue = IssueFormatter.coordinate_error_to_dict(
                            x, "Coordinate provided is outside England"
                        )
                        geoy_issue = IssueFormatter.coordinate_error_to_dict(
                    ...
                    issues[row][field] = IssueFormatter.generate_issue_message(
                        field, issues_with_field
                    )
